    ...
    for system in all_systems:
        ...
        if not df.empty:
            if OUTCOME:
                ...
                plt.savefig(os.path.join(result_dir_sys, '_'.join([system.name, 'wlt_vs_time_outcome_round1.pdf'])),
                            format='pdf', bbox_inches='tight')
                plt.savefig(os.path.join(result_dir_sys, '_'.join([system.name, 'wlt_vs_time_outcome_round1.svg'])),
                ...
                plt.savefig(os.path.join(result_dir_sys, '_'.join([system.name, 'wlt_vs_time_round1.pdf'])),
                            format='pdf', bbox_inches='tight')
                plt.savefig(os.path.join(result_dir_sys, '_'.join([system.name, 'wlt_vs_time_round1.svg'])),
                            format='svg', bbox_inches='tight')
                plt.show()
    ...
    for system in all_systems:
        ...
        if not df.empty:
            if OUTCOME:
                ...
                plt.savefig(os.path.join(result_dir_sys, '_'.join([system.name, 'wlt_vs_time_outcome_round2.pdf'])),
                            format='pdf', bbox_inches='tight')
                plt.savefig(os.path.join(result_dir_sys, '_'.join([system.name, 'wlt_vs_time_outcome_round2.svg'])),
                            format='svg', bbox_inches='tight')
                plt.show()
                ...
                df.transpose().cumsum().plot.line()
                plt.title(' - '.join([system.name, 'Cumulative Wins, Losses, and Ties']))
                plt.savefig(os.path.join(result_dir_sys, '_'.join([system.name, 'wlt_vs_time_round2.pdf'])),
                            format='pdf', bbox_inches='tight')
                plt.savefig(os.path.join(result_dir_sys, '_'.join([system.name, 'wlt_vs_time_round2.svg'])),
                            format='svg', bbox_inches='tight')
                plt.show()
