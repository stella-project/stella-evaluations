...
def main():
    ...
    ranking_sessions = sessions.select(sessions.c.system_ranking.in_(ranking_systems)).where(and_(sessions.c.start > ROUND_01_START, sessions.c.end < ROUND_02_END)).execute().fetchall()
    ...
    recommendation_sessions = sessions.select(sessions.c.system_recommendation.in_(recommendation_systems)).where(and_(sessions.c.start > ROUND_01_START, sessions.c.end < ROUND_02_END)).execute().fetchall()
